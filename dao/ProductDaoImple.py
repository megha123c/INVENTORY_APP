...
class ProductDaoImplementation(ProductDaoService):
    ...
    def display_all_products(self) -> List[Product]:
        ...
        try:
           ...
           for row in rows:
               products.append(Product(productid=row["productid"],productname=row["productname"],
                                    unitprice=row["unitprice"],categoryid=row["categoryid"],
                                    manufacturedate=row["manufacturedate"],is_active=row["isActive"]))
        except Exception as e:
    ...
    def update_product(self,product:Product,product_id:int) ->bool:
        try:
            cursor = self.conn.cursor(dictionary=True)
            cursor.execute(self.UPDATE_PRODUCT,(product.get_productname(),product.get_unitprice(),product_id))
            self.conn.commit()
            return cursor.rowcount == 1
        except Exception as e:
